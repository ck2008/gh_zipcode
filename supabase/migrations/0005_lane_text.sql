...
language sql stable security definer set search_path = postal, pg_temp as $$
  with direct_match as (
    select ps.zip_code, ps.city_name, ps.district_name, ps.street_name, ps.sector, ps.source_detail
    from postal.post_street ps
    where p_house_number <> -1
      and ps.city_name = p_city and ps.district_name = p_district and ps.street_name = p_street
      and (p_sector = '' or ps.sector = '0' or ps.sector = p_sector)
      and ps.record_type = 2 and ps.lane ~ '^[0-9]+$'
      and p_house_number between ps.house_number and ps.lane::integer
      and ps.number_type = p_number_type
  )
